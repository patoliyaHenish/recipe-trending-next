...
// Find the line that contains ag-theme-alpine (line 700ish)
let startIndex = -1;
let endIndex = -1;

for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('ag-theme-alpine')) {
    // The <Box starts on the previous line or this line.
    startIndex = i - 1; // Assuming line 699 is `<Box`
  }
  if (startIndex !== -1 && lines[i].includes('/>') && lines[i-1].includes('overlayNoRowsTemplate')) {
    endIndex = i + 1; // The `</Box>` line
    break;
  }
}
...
`;
  lines.splice(startIndex, endIndex - startIndex + 1, tableCode);
  fs.writeFileSync(targetPath, lines.join('\n'), 'utf8');
  console.log("Successfully replaced AG Grid Box!");
} else {
  console.log("Could not find AG Grid Box", { startIndex, endIndex });
}
