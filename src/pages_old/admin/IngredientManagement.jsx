...
import { Box, Button, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, TextField, Typography, Autocomplete, Chip, FormControl, Select, MenuItem, Pagination, useMediaQuery, Tooltip } from '@mui/material';
...
const IngredientManagement = () => {
  ...
  const columnDefs = useMemo(() => {
          ...
          return (
            <Box className="flex gap-2 justify-center items-center h-full">
              <Tooltip title="View" arrow>
                <IconButton
                  size="small"
                  onClick={() => handleOpenView(ingredient)}
                  sx={{
              ...
              {canDelete && (
                <Tooltip title="Delete" arrow>
                  <IconButton
                    size="small"
                    onClick={() => handleOpenDeleteConfirm(ingredient.ingredient_id)}
                    sx={{
    ...
    return defs;
  }, [isDarkMode, canCreate, canUpdate, canDelete, onEditClick]);
        ...
        <Box sx={{ overflowX: 'auto', width: '100%', flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          >
            <AgGridReact
              ...
              overlayNoRowsTemplate='<span class="ag-overlay-no-rows-center">No ingredients found</span>'
            />
          </Box>
        </Box>
