          ...
          <>
            <Box sx={{ 
              ...
              <Box sx={{ gridColumn: 'span 2' }}>
                <DetailRow label="Recipe Name" value={recipeNote.recipe_name} />
              </Box>
              
              <Box sx={{ gridColumn: 'span 2' }}>
                <DetailRow label="Note Message" value={recipeNote.message} />
              </Box>

              <DetailRow label="Status">
                  <Chip
                    label={statusLabel(recipeNote.status || 'pending')}
                    size="small"
                    sx={{
                      fontWeight: 700,
                      fontSize: '0.7rem',
                      height: '22px',
                      color: statusColors[recipeNote.status] || (isDarkMode ? "#e5e7eb" : "#374151"),
                      backgroundColor: isDarkMode ? 'rgba(255,255,255,0.04)' : '#f3f4f6',
                      border: `1px solid ${statusColors[recipeNote.status] ? statusColors[recipeNote.status] + '33' : (isDarkMode ? '#e5e7eb33' : '#37415133')}`,
                    }}
                  />
              </DetailRow>

              <DetailRow label="Commented By">
                 {recipeNote.commenter_name ? (
                      <Box>
                         ...
                         <Typography variant="caption" sx={{ color: isDarkMode ? '#9ca3af' : '#6b7280', display: 'block' }}>
                             {recipeNote.commenter_email}
                         </Typography>
                      </Box>
                  ) : (
                      <Typography variant="body2" sx={{ fontWeight: 600, color: isDarkMode ? '#e5e7eb' : '#111827' }}>
                          ID: {recipeNote.commenter_id || 'Unknown'}
                      </Typography>
                  )}
              </DetailRow>

              <DetailRow label="Category" value={recipeNote.category_name} />
              <DetailRow label="Sub-category" value={recipeNote.sub_category_name} />

              {recipeNote.created_at && (
                  <DetailRow label="Created At">
                      <Typography variant="body2" sx={{ fontWeight: 600, color: isDarkMode ? '#e5e7eb' : '#111827' }}>
                        {new Date(recipeNote.created_at).toLocaleString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true })}
                      </Typography>
                  </DetailRow>
              )}

              {recipeNote.updated_at && (
                  <DetailRow label="Updated At">
                      <Typography variant="body2" sx={{ fontWeight: 600, color: isDarkMode ? '#e5e7eb' : '#111827' }}>
                        {new Date(recipeNote.updated_at).toLocaleString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true })}
                      </Typography>
                  </DetailRow>
              )}
            </Box>
...
export default ViewRecipeNoteDialog;
